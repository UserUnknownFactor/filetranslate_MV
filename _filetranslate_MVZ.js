...
const IGNORE_CHARS = getArray(parameters['Ignored Characters'] || ["]",  ")", "、", "。", "，", "。", "！", "？", "!", "?", "”", "’", "）", "】"]);
...
const LANGUAGE_MAPPING = parameters["Languages Data"] ? JSON.parse(parameters["Languages Data"]) : {
	...
	'':  { name: "English", flagX: 0, flagY: 2912, buttonText: "Language", BFS: 28,  MFS: 28, HFS: 28 },
	//"_en": { name: "English", flagX: 0, flagY: 2912, buttonText: "Language" },
	"_jp": { name: "Japanese", flagX: 0, flagY: 3968, buttonText: "Language", disable: true },
};
...
if (CHANGE_FONTS) {
	if (!MV_MODE) { // MZ
		// Default MZ font sizes: Most windows = 26~28
		function wrapResetFontSettings(windowClass, fontSizeKey) {
			...
			windowClass.prototype.resetFontSettings = function() {
				...
				const fontSize = LANGUAGE_MAPPING[ConfigManager.language][fontSizeKey];
				if (fontSize >= 1)
		...
		// Battle-related
		wrapResetFontSettings(Window_BattleLog, 'BFS');	// Battle messages
		wrapResetFontSettings(Window_BattleStatus, 'BSS'); // Party status in battle
		wrapResetFontSettings(Window_BattleActor, 'BAS'); // Actor selection
		wrapResetFontSettings(Window_BattleEnemy, 'BES'); // Enemy selection
		wrapResetFontSettings(Window_BattleSkill, 'BSS'); // Skill selection
		wrapResetFontSettings(Window_BattleItem, 'BIS');  // Item selection
	
		// Menu-related
		wrapResetFontSettings(Window_Message, 'MFS');	 // Dialog messages
		wrapResetFontSettings(Window_Help, 'HFS');		// Help/description text
		wrapResetFontSettings(Window_MenuStatus, 'MSS');  // Party status in menu
		wrapResetFontSettings(Window_Status, 'STS');	  // Status screen
		wrapResetFontSettings(Window_ItemList, 'ILS');	// Item list
		wrapResetFontSettings(Window_SkillList, 'SLS');   // Skill list
		wrapResetFontSettings(Window_EquipStatus, 'EQS'); // Equipment status
		wrapResetFontSettings(Window_ShopBuy, 'SBS');	 // Shop buy window
		wrapResetFontSettings(Window_ShopSell, 'SSS');	// Shop sell window
		wrapResetFontSettings(Window_NameInput, 'NIS');   // Name input
		wrapResetFontSettings(Window_Options, 'OPS');	 // Options menu
		wrapResetFontSettings(Window_SavefileList, 'SFL'); // Save file list
		wrapResetFontSettings(Window_GameEnd, 'GES');	 // Game end menu
	} else { // MV
		// Default MV font sizes: Most windows = 28
		...
		// Same windows as above but using MV's standardFontSize
		wrapStandardFontSize(Window_BattleLog, 'BFS');
		wrapStandardFontSize(Window_BattleStatus, 'BSS');
		wrapStandardFontSize(Window_BattleActor, 'BAS');
		wrapStandardFontSize(Window_BattleEnemy, 'BES');
		wrapStandardFontSize(Window_BattleSkill, 'BSS');
		wrapStandardFontSize(Window_BattleItem, 'BIS');
	
		wrapStandardFontSize(Window_Message, 'MFS');
		wrapStandardFontSize(Window_Help, 'HFS');
		wrapStandardFontSize(Window_MenuStatus, 'MSS');
		wrapStandardFontSize(Window_Status, 'STS');
		wrapStandardFontSize(Window_ItemList, 'ILS');
		wrapStandardFontSize(Window_SkillList, 'SLS');
		wrapStandardFontSize(Window_EquipStatus, 'EQS');
		wrapStandardFontSize(Window_ShopBuy, 'SBS');
		wrapStandardFontSize(Window_ShopSell, 'SSS');
		wrapStandardFontSize(Window_NameInput, 'NIS');
		wrapStandardFontSize(Window_Options, 'OPS');
		wrapStandardFontSize(Window_SavefileList, 'SFL');
		wrapStandardFontSize(Window_GameEnd, 'GES');
	}
}
...
if (ENABLE_WORDWRAP) {
	...
	Window_Message.prototype.needsNewLine = function(textState) {
		...
		if (IGNORE_CHARS.includes(currentChar) && !IGNORE_CHARS.includes(nextChar))
			return false;
	...
	(MV_MODE ? wordwrap_MV : wordwrap_MZ)();
}
